  ...
  return (
    <ScrollArea className="h-[calc(100vh-180px)]">
      <div className="space-y-4 p-4">
        {reports.length === 0 ? (
          <Card>
            <CardContent className="p-6 text-center text-muted-foreground">No reports generated yet</CardContent>
          </Card>
        ) : (
          reports.map((report) => (
            <Card
...
function StatusBadge({ status }: { status: "generating" | "complete" | "failed" }) {
  const variants = {
